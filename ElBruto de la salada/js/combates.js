...
import { escribirLog, actualizarUI, bloquearBotones, pausar, renderizarMiHub, animarAvatar, animarShake, mostrarTextoFlotante, flashCritico, mostrarVFX, mostrarCombo, lanzarChispas, SFX } from "./ui.js";
...
export async function simularCombateVisual(jugadorA, jugadorB) {
    ...
    while (clonA.vidaActual > 0 && clonB.vidaActual > 0) {
        ...
        if (atacante.mascotaActiva) {
            ...
            if (dañoMascota > 0) {
                ...
                if (logsDefensaMascota.length > 0) { msgMascota += logsDefensaMascota.join(""); }
    ...
    ganadorReal.estadisticasExtra.rachaDerrotas = 0; 
    ...
    for (let accion of lineaDeTiempo) {
        ...
        if (accion.hpB < hpAnteriorB) {
            lanzarChispas(2, 15);
            let daño = hpAnteriorB - accion.hpB; 
